...
```{r include=FALSE}
dmcognigen_pk_requirements <- dmcognigen_pk_requirements %>% 
...
For example, below is a decode table for a variable `RFCAT` (`r attr(dmcognigen_cov$RFCAT, "label")`). 
...
```{r}
extract_decode_tbls(
  variable_name = dmcognigen_pk_requirements$variable_name,
  decode = dmcognigen_pk_requirements$format_decode
)
```
...
Like how this `dmcognigen_cov` data set includes the pair of variables `RACEN` & `RACEC`, along with the original source variable `RACE`.
```{r}
dmcognigen_cov %>%
  extract_decode_tbls_from_data(
    lvl_to_lbl = list(
...
pk_numeric %>% 
  set_decode_factors(
    decode_tbls = dmcognigen_pk_requirements, 
    new_names = list(
      "{var}FCT",
      RACEN = "RACEN", 
      SEXF = "SEXFC" 
...
pk_numeric_with_label_vars <- pk_numeric %>% 
  join_decode_labels(dmcognigen_pk_requirements)
```
...
dmcognigen_pk %>% 
  ...
  join_decode_levels(dmcognigen_pk_requirements, lvl_to_lbl = list(RACEN = "RACEC", "{var}C")) %>% 
  cnt(RACEN, RACEC, SEXF, SEXFC, n_distinct_vars = ID)
```
